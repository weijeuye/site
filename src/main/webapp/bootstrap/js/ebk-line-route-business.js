...
$(function() {
    ...
    $document.on("click", ".JS_line_travel_view_log", function() {
        ...
        if (isEmpty(routeId)) {
            $.alert("请先选择行程天数，并点击保存。");
            return;
        }
        var objectType='PROD_LINE_ROUTE';
        // new xDialog("/vst_ebooking/ebooking/findLogList.do?objectId="+routeId+"&objectType="+objectType,{},{title:"日志",iframe:true,width:1000,hight:500,scrolling:"yes"});
        var param = 'objectId='+routeId+'&objectType=PROD_LINE_ROUTE&sysName=VST';
        showLogDialog = new xDialog("http://super.lvmama.com/lvmm_log/bizLog/findLogList?"+param,{},{title:"日志", iframe:true,width:1000,hight:500,iframeHeight:680,scrolling:"yes"});
    });
    ...
    function loadLineRoute($giForms, productId, productType, lineRouteId) {
        ...
        var requestUrl = "/vst_ebooking/ebooking/dujia/group/route/showLineRoute.do?productId="+productId+"&productType="+productType+"&lineRouteId="+lineRouteId;
    ...
    function loadCostInclude($giForms, productId, productType, lineRouteId) {
        ...
        if (productType == 'INNERSHORTLINE' || productType == 'INNERLONGLINE' || productType == 'INNER_BORDER_LINE') {// 国内-短线 or 国内-长线
            requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costinclude/showCostIncludeInner.do?productId="+productId+"&productType="+productType+"&lineRouteId="+lineRouteId;
            $fillPosition = $giForms.find(".JS_inner_include_internal");
        } else if (productType == 'FOREIGNLINE') {// 出境/港澳台
            requestUrl="/vst_ebooking/ebooking/dujia/group/route/costinclude/showCostIncludeOutside.do?productId="+productId+"&productType="+productType+"&lineRouteId="+lineRouteId;
    ...
    function loadCostExclude($giForms, productId, productType, lineRouteId) {
        ...
        var $fillPosition = "";
        if (productType == 'INNERSHORTLINE' || productType == 'INNERLONGLINE' || productType == 'INNER_BORDER_LINE') {// 国内-短线 or 国内-长线
            requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costexclude/showCostExcludeInner.do?productId="+productId+"&productType="+productType+"&lineRouteId="+lineRouteId;
            $fillPosition = $giForms.find(".JS_inner_not_include_internal");
        } else if (productType == 'FOREIGNLINE') {// 出境/港澳台
            requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costexclude/showCostExcludeOutside.do?productId="+productId+"&productType="+productType+"&lineRouteId="+lineRouteId;
    ...
    function loadActivityRecommended($giForms, productId, lineRouteId,categoryId) {
        var requestUrl = "/vst_ebooking/ebooking/dujia/group/route/showActivityRecommended.do?productId="+productId+"&lineRouteId="+lineRouteId+"&categoryId="+categoryId;
    ...
    function saveLineRoute($this){
        ...
        if ($giTabs.length > 0) {
            ...
            if (noSaveRouteName) {
                ...
                $.alert(noSaveRouteName + "尚未保存，请先将其保存");
                return;
            }
        }
    ...
    function copyRoute($this) {
        ...
        $.confirm("确认要复制\""+routeName+"\"？", function() {
            $this.text("复制中...");
    ...
    function invalidRoute($this) {
        ...
        //如果将行程由 有效->无效时给出提示
        if (routeStatus == "Y") {
            $.confirm("设置无效，对应的\""+routeName+"\"的时间价格表也会无效，确认操作？", function() {
    ...
    $document.on("click", ".JS_cost_include_preview", function () {
        var requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costinclude/previewCostInclude.do?preview=inner";
        var $form = $(this).closest(".costIncludeInnerForm");
        $.ajax({
            url: requestUrl,
            data: $form.serialize(),
            ...
            dataType: "JSON",
            success: function(result){
                $(".gi-modal-p-title").html("费用包含(国内)");
                $(".gi-modal-p-content").html(result.message);
    ...
    $document.on("click", ".JS_cost_include_outside_preview", function () {
        var requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costinclude/previewCostInclude.do?preview=outside";
        var $form = $(this).closest(".costIncludeOutsideForm");
        $.ajax({
            url: requestUrl,
            data: $form.serialize(),
    ...
    $document.on("click", ".JS_cost_exclude_preview", function () {
        var requestUrl = "/vst_ebooking/ebooking/dujia/group/route/costexclude/previewCostExclude.do?preview=inner";
        var $form = $(this).closest(".costExcludeInnerForm");
        $.ajax({
            url: requestUrl,
            data: $form.serialize(),
            ...
            dataType: "JSON",
            success: function(result){
                $(".gi-modal-p-title").html("费用不包含(国内)");
                $(".gi-modal-p-content").html(result.message);
                $(".gi-modal, .gi-modal-overlay").show();
            },
            error: function() {
    ...
    $document.on("click", ".JS_cost_exclude_outside_preview", function () {
        ...
        $.ajax({
            ...
            dataType:"JSON",
            success : function(result){
                $(".gi-modal-p-title").html("费用不包含(出境)");
                $(".gi-modal-p-content").html(result.message);
                $(".gi-modal, .gi-modal-overlay").show();
            },
            error: function() {
    ...
    $document.on("click",".gi-modal-view .gi-modal-close",function() {
        $(".gi-modal, .gi-modal-overlay").hide();
    });

    $document.on("click", ".gi-modal-view .gi-modal-close-btn", function () {
        $(".gi-modal, .gi-modal-overlay").hide();
    });
