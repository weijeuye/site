...
public class Pagination {
    ...
    public static String pageSupplierForES(Page<?> page) {
        if ((page != null) && (page.getTotalPageNum() > 0)) {
            Pagination pagination = new Pagination();
            pagination.setPageSize(page.getPageSize());
            pagination.setTotalPage(page.getTotalPageNum());
    ...
    public static String pageSupplierForEBKNew(Page<?> page) {
        if ((page != null) && (page.getTotalPageNum() > 0)) {
            Pagination pagination = new Pagination();
            pagination.setPageSize(page.getPageSize());
            pagination.setTotalPage(page.getTotalPageNum());
	...
	public void createTag(int type) {
		try {
			...
			case 4: // 模式3 (箭头样式,首页,前页,后页,尾页) (only IE)
				...
				if (nextPage > this.totalPage) {
					str += "<span title='下一页'><strong>></strong></span>";
			...
			case 9: // 模式2 (前后缩略,页数,首页,前页,后页,尾页)
				...
				if (nextPage > this.totalPage) {
					str += "<span id=\"last\" title='下一页'>下一页</span>";
			...
			case 13:
				...
				break;
			case 14:
				str = createFormPaginationForES();
				break;
			case 15:
				str = createFormPaginationForEbkNew();	//ebk改版后，分页面走ajax
				break;
			}
		...
		try {
			...
			case 0:
				...
				if (prevPage < 1) {
					...
					//str += "<span title='到第一页'><a href='" + this.toPage(1) + "'>&#171;</a></span>";
					str += "<span title='到第一页'><a onclick='asyncQuery('" + this.toPage(1) + "')'>&#171;</a></span>";
					//str += "<span title='上一页'><a href='" + this.toPage(prevPage) + ");'>&#139;</a></span>";
					str += "<span title='上一页'><a onclick='asyncQuery('" + this.toPage(prevPage) + "');'>&#139;</a></span>";
				}
				for (long i = 1; i <= this.totalPage; i++) {
					if (i > 0) {
						if (i == this.page) {
							...
							//str += "<span title='第 " + i + "页'><a href='" + this.toPage(i) + "'>[" + i + "]</a></span>";
							str += "<span title='第 " + i + "页'><a onclick='asyncQuery('" + this.toPage(i) + "')'>[" + i + "]</a></span>";
						}
					}
				}
				if (nextPage > this.totalPage) {
					...
					//str += "<span title='下一页'><a href='" + this.toPage(nextPage) + "'>&#155;</a></span>";
					str += "<span title='下一页'><a onclick='asyncQuery('" + this.toPage(nextPage) + "')'>&#155;</a></span>";
					//str += "<span title='到最后一页'><a href='" + this.toPage(this.totalPage) + " ');'>&#187;</a></span>";
					str += "<span title='到最后一页'><a onclick='asyncQuery('" + this.toPage(this.totalPage) + "');'>&#187;</a></span>";
				}
				str += "</span><br/>";
			...
			case 1: // 模式1 (前后缩略,页数,首页,前页,后页,尾页)
				...
				if (prevPage < 1) {
					...
					str += "<span id=\"last\" title='上一页' >上一页</span>";
					  ...
					  str += "<span id=\"pages\" title='到第一页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(1) + "\')\">首页</a></span>";
					  
					str += "<span id=\"pages\" title='上一页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(prevPage) +"\')\">上一页</a></span>";
				}
				if (this.page != 1) {
					str += "<span id=\"test\"  title='到第一页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(1) + "\')\">1</a></span>";
				}
				...
				for (long i = this.page - 2; i <= endPage1; i++) {
					if (i > 0) {
						if (i == this.page) {
							...
							if ((i != 1) && (i != this.totalPage)) {
								str += "<span id=\"test\" title='第 " + i + "页'><a  style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(i) +"\')\">" + i + "</a></span>";
							}
						}
					}
				}
				...
				if (this.page != this.totalPage) {
					str += "<span id=\"test\"  title= '第 " + this.totalPage + "页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(this.totalPage) +"\')\">" + this.totalPage + "</a></span>";
				}
				...
				if (nextPage > this.totalPage) {
					...
					str += "<span id=\"pages\"  title='下一页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(nextPage) + "\')\">下一页</a></span>";

					str += "<span id=\"toLast\" title='最后一页'><a style=\"border:1px solid #ddd;\" onclick=\"asyncQuery(\'" + this.toPage(this.totalPage) + "\')\">最后一页</a></span>";
				}
				str += "</span><br/>";
				break;
			case 2:
				...
				if (prevPage < 1) {
					str += "<a href=\"#\" title=\"上一页\" class=\"PrevPage\" >上一页</a>";
					     ...
					     str += "<a  onclick=\"asyncQuery(\'" + this.toPage(prevPage) + "\')\" page=\""+prevPage+"\" title=\"上一页\" class=\"PrevPage\" >上一页</a>";
				}
				if (this.page != 1) {
					str += "<a  onclick=\"asyncQuery(\'"+this.toPage(1)+"\')\"   page=\""+1+"\" title=\"到第一页\" class=\"PageLink\" >1</a>";
				}
				...
				for (long i = this.page - 5; i <= endPage2; i++) {
					if (i > 0) {
						if (i == this.page) {
							...
							if ((i != 1) && (i != this.totalPage)) {
								  str += "<a onclick=\"asyncQuery(\'" + this.toPage(i) + "\')\"  page=\""+i+"\" class=\"PageLink\" title='第 " + i + "页'>" + i + "</a>";
							}
						}
					}
				}
				...
				if (this.page != this.totalPage) {
					str += "<a  onclick=\"asyncQuery(\'"+this.toPage(this.totalPage)+"\')\"  page=\""+totalPage+"\" class=\"PageLink\" title= '第 " + this.totalPage + "页'>" + this.totalPage + "</a>";
				}
				...
				if (nextPage > this.totalPage) {
					...
					str += "<a onclick=\"asyncQuery(\'"+this.toPage(nextPage)+"\')\"  page=\""+nextPage+"\" title=\"下一页\" class=\"NextPage PageLink PageLink_page\" >下一页</a>";
				}
				str += "</span><br/>";
				str += "</div>";
				str += "</div>";
	...
	private String createFormPagination2() {
		...
		sb.append("<a href=\"javascript:void(0)\" type=\"button\" class=\"btn btn-gray btn-small "+pageAName+"\" total='" + totalPage + "' result='gogo'>确定</a></span>");
	 ...
	 * @return
	 */
	private String createFormPaginationForES() {
		StringBuffer sb = new StringBuffer();
		createAction();
		sb.append("<div class=\"paging graystyle\">");
		sb.append("<form action='");
	 ...
	 * 对EBK改版 ajax分页
	 */
	private String createFormPaginationForEbkNew() {
		StringBuffer sb = new StringBuffer();
		createAction();
		//sb.append("<div class=\"pages JS_pages\" data=\"pro_all_products_tab1_result\">");
		sb.append("<span class='pages_info'>共<b>").append(totalResultSize).append("</b>条记录，每页显示").append(pageSize).append("条记录</span>");
		if (hasPrev()) {
			sb.append("<a href='javascript:void(0)'  class=\"prevpage\" page=\"").append(page - 1).append("\">&lt; 上一页</a>");
		}
		long startPage = 1;
		long endPage = totalPage;
		if (totalPage > 10) {
		...
		for (long i = startPage; i <= endPage; i++) {
			if (i == page) {
				sb.append("<a href=\"javascript:;\" class=\"active\" page=\"").append(i).append("\">").append(i).append("</a>");
			} else {
			    sb.append(createPage(i));
			}
		}
		sb.append("<span class='pagemore'>...</span>");
		if (hasNext()) {
			sb.append("<a href='javascript:void(0)'  class=\"nextpage\" page=\"").append(page + 1).append("\">下一页 &gt;</a>");
		}
		sb.append("到第<input class=\"pages_input\" type=\"text\">页<input value=\"确定\" class=\"pages_btn\" type=\"button\">");
	...
	public String doAsyncStartTag() {
		try {
			initPage();
			str += "<div id='pages_' class='pages' style='" + this.cssStyle + "'> ";
			this.createAsyncTag(this.getMode());
			str += "</div>";
		} catch (Exception ex) {
			logger.error(ExceptionFormatUtil.getTrace(ex));
		}
