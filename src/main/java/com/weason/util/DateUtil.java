...
public class DateUtil {
	 ...
	 * @return
	 */
	public static IntersectionDate getIntersectionDate(Date startDate, Date endDate, Date sd, Date ed) {
		if (null == startDate) {
			throw new RuntimeException("取两个日期范围的交集，日期范围1的开始时间不能为空");
		}
		if (null == endDate) {
			throw new RuntimeException("取两个日期范围的交集，日期范围1的结束时间不能为空");
		}
		if (null == sd) {
			throw new RuntimeException("取两个日期范围的交集，日期范围2的开始时间不能为空");
		}
		if (null == ed) {
			throw new RuntimeException("取两个日期范围的交集，日期范围2的结束时间不能为空");
		}
		IntersectionDate intersectionDate = null;
		// 结束时间在开始时间之前
		...
		intersectionDate = new IntersectionDate();
		if (sd.before(startDate) && ed.after(endDate)) {
			intersectionDate.setStartDate(startDate);
			intersectionDate.setEndDate(endDate);
			return intersectionDate;
		}
		if (startDate.before(sd) && endDate.after(ed)) {
			intersectionDate.setStartDate(sd);
			intersectionDate.setEndDate(ed);
			return intersectionDate;
		}
		if (sd.after(startDate) && sd.before(endDate)) {
			intersectionDate.setStartDate(sd);
			intersectionDate.setEndDate(endDate);
			return intersectionDate;
		}
		if (ed.before(endDate) && ed.after(startDate)) {
			intersectionDate.setStartDate(startDate);
			intersectionDate.setEndDate(ed);
			return intersectionDate;
		}
	 ...
	 * 
	 * @param delayWeek
	 *            为推迟的周数，0本周，-1向前推迟一周，1下周，依次类推
	 * @param whichDay
	 *            想周几，这里就传几,如：Calendar.MONDAY
	 * @return
	 */
	...
	public static final String PATTERN_yyMMdd_2 = "yyMMdd";
	...
	public static int getDaysBetween(Date startDate, Date endDate) {
		...
		d2.setTime(endDate);
		if (d1.after(d2)) { // swap dates so that d1 is
							// start and d2 is end
			Calendar swap = d1;
			d1 = d2;
			d2 = swap;
	...
	public static Date mergeDateTimeAddYear(Date date, Integer years) {
		Calendar dateCalendar = Calendar.getInstance();
		dateCalendar.setTime(date);
		dateCalendar.set(dateCalendar.YEAR, dateCalendar.get(Calendar.YEAR) + years);
		return dateCalendar.getTime();
	}
	...
	public static Date mergeDateTimeAddMonth(Date date, Integer Months) {
		Calendar dateCalendar = Calendar.getInstance();
		dateCalendar.setTime(date);
		dateCalendar.set(dateCalendar.MONTH, dateCalendar.get(Calendar.MONTH) + Months);
		return dateCalendar.getTime();
	}
	...
	public static void main(String[] args) throws Exception {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		Date startDate = sdf.parse("2016-3-1");
		Date endDate = sdf.parse("2016-12-31");
		Date sd = sdf.parse("2016-3-1");
		Date ed = sdf.parse("2016-12-31");
		IntersectionDate intersectionDate = getIntersectionDate(startDate, endDate, sd, ed);
		if (null == intersectionDate) {
			System.out.println("null = intersectionDate ");
			return;
		}
		System.out.println(sdf.format(intersectionDate.getStartDate()));
		System.out.println(sdf.format(intersectionDate.getEndDate()));
	}
	...
	public static String convertDate4(Date specDate) {
		return new SimpleDateFormat(PATTERN_yyMMdd_2).format(specDate);
	}
	...
	/**
	 * 
	 * @Title: DcDay_minOfDay @Description:
	 *         传入的时间，加上给定的分钟数 @param @param
	 *         date @param @param minstes @param @return
